...
def call_ai(messages: list) -> tuple[str, bool]:
    global active_model
    headers = {"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"}
    payload = {"model": active_model, "messages": messages, "max_tokens": 400}
    try:
        r = requests.post(URL_CHAT, headers=headers, json=payload, timeout=15)
        if r.status_code == 429 and active_model == MODEL_PRIMARY:
            ...
            speak("Rate limit reached on main model. Switching to fallback.")
            active_model = MODEL_FALLBACK
            payload["model"] = active_model
            r = requests.post(URL_CHAT, headers=headers, json=payload, timeout=15)
        if r.status_code != 200:
            return f"API Error {r.status_code}: {r.text[:200]}", False
