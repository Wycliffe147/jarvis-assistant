...
get_battery_diagnostics(target_ip) - READ-ONLY. Reports battery STATE: voltage, temperature, technology, health, charging status. Does NOT identify what is consuming power or causing drain (Android doesn't expose per-app battery usage through this command). If asked "why is my battery draining" or "what's using my battery", report these state facts but be explicit that this does not identify a specific cause -- suggest checking Settings > Battery > Battery usage on the device for an app-level breakdown.
